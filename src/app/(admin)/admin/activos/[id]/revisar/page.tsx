...
export default async function AssetReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function setScheduleAction(formData: FormData) {
    ...
    await setAssetReviewSchedule(u, id, intervalValue, intervalUnit);
    redirect(`/admin/activos/${id}/revisar`);
  }
  ...
  return (
    <div className="max-w-4xl space-y-4">
          ...
          <form action={submitReviewAction} className="space-y-4">
            ...
            <Field>
              ...
              <Textarea
                id="comments"
                name="comments"
                placeholder="Describe el estado del activo, observaciones o motivo de inoperatividad..."
                className="h-32"
                maxLength={1000}
              />
            </Field>
      ...
      {!asset.reviewIntervalValue && (
        <Card>
          <CardHeader><CardTitle>Configurar intervalo de revisión</CardTitle></CardHeader>
          <CardBody>
            <form action={setScheduleAction} className="space-y-4">
              <p className="text-sm text-slate-600 mb-4">
                Define cada cuánto tiempo se debe revisar este activo. Los encargados recibirán alertas automáticas.
              </p>
              ...
              <div className="flex gap-2 justify-end">
                <Button type="submit" variant="info">Configurar intervalo</Button>
              </div>
            </form>
          </CardBody>
        </Card>
