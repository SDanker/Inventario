...
export default async function UnitTransfersPage() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/unidad/traslados/nuevo">
          <Button>Nuevo traslado</Button>
        </Link>
      </div>

      <Card>
        <CardHeader><CardTitle>{transfers.length} traslados</CardTitle></CardHeader>
        <CardBody>
          {transfers.length === 0 ? (
            <EmptyState>Sin traslados. Crea el primero.</EmptyState>
          ) : (
            <Table>
              <THead>
                <TR>
